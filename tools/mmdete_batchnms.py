...
out_dir = "/home/chenzhen/code/detection/datasets/nms_save-5"
...
def vis(draw_img, bbox, labels, label_color, xywh=False):
    if xywh:
        x0, y0, w, h = map(lambda x: int(round(x)), bbox)
        x1, y1 = x0 + w, y0 + h
    else:
    ...
    txt_size = cv2.getTextSize(labels, font, 0.4, 1)[0]
    cv2.rectangle(draw_img, (x0, y0), (x1, y1), label_color, 2)

    txt_bk_color = label_color
    cv2.rectangle(
        draw_img,
        (x0, y0 + 1),
    ...
    cv2.putText(draw_img, label, (x0, y0 + txt_size[1]), font, 0.4, txt_color, thickness=1)
    ...
    if len(files) != 0:
        for file in files:
            ...
            for i, (bbox, score, label) in enumerate(zip(bbox.numpy().astype(np.int32), score.numpy(), label.numpy())):
                ...
                img = vis(img, bbox, f'{label_text}', PALETTE[int(label)], xywh=False)
