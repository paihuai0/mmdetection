...
########################### DaTang Mobile YoloX v1.1 2022-10-08 ###################################
...
model = dict(
    ...
    bbox_head=dict(
        type='YOLOXHead_DT', num_classes=8, in_channels=128, feat_channels=128),
    train_cfg=dict(
...
########################### data loading pipeline 2022-10-08 #########################################
...
train_pipeline = [
    ...
    dict(type='FilterAnnotations', min_gt_bbox_wh=(5, 5), keep_empty=False), # v1.1-1  v1.1-2
    dict(type='DefaultFormatBundle'),
    dict(type='Collect', keys=['img', 'gt_bboxes', 'gt_labels','gt_bboxes_ignore'],meta_keys=('filename', 'ori_filename', 'ori_shape',
                            'img_shape', 'pad_shape', 'scale_factor', 'flip',
                            'flip_direction', 'img_norm_cfg','gt_occs')) #v1.1-1  v1.1-2
]
...
train_dataset = dict(
    ...
    dataset=dict(
        type=dataset_type,
        ann_file=data_root + 'ann/train_demo.json',
...
########################### learning parameters 2022-10-08 ###########################################
# optimizer
# default 8 gpu
# optimizer
...
# auto_scale_lr = dict(enable=False, base_batch_size=64)  # revise according to real situation 2022-10-08
